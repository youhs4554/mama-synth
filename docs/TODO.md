...
## Active goal: Phase 1A (pix2pixHD fine-tune) + Phase 2 (latent diffusion) sweep to stable performance (2026-05-30)

User direction (`/goal`): 설계상 Phase 1A와 Phase 2를 모두 진행하고, 안정적 성능을 얻을 때까지 실험 sweep을 지속한다. GPU 0 (A4500) only; one GPU job at a time. Promotion only when a credible fold's four metric groups are jointly satisfied vs baseline.

### Phase 1A — pix2pixHD fine-tune, complete the schedule + metric-based selection + NACT-64 LOSO
- [x] Add periodic checkpoint snapshots to `finetune_pix2pixhd.py` (`--ckpt_every`) for metric-based selection (val_l1 is a poor proxy).
- [~] Run a fresh longer fine-tune (40 epochs, batch 4, warm-start 00023) into `experiments/phase1b/finetune_pix2pixhd_v2/` on GPU 0, durable log (bg `byqshe8lb`, ~82s/epoch, snapshots e0/e5/...; verified GPU 0 only, GPU 1 idle).
  - **BLOCKED — server hard power-cut at 2026-05-30 10:58:52 (boot -1 ended), ~62s auto power-on.** Diagnosis: load-correlated power cut (no kernel panic/thermal/OOM/MCE before the cut; no UPS), reproduced with ONLY GPU 0 loaded → refutes the concurrency-only theory. PCIe AER *correctable* timeout on PEX 8747 ~3 min prior. See CLAUDE.md GPU rule update + memory `server-power-crash-gpu-load`.
  - **Recovered intact:** reached epoch 19/40 (crash ~1 min into e20). G_L1 0.665→0.437 (past v1's e16). All 6 checkpoints load cleanly (weights_only, 56 keys): `30_net_G_last.pth`=e19, `30_net_G.pth`=best_val_l1, snapshots e0/e5/e10/e15. Usable for n=64 LOSO eval once GPU is safe again.
  - **Do NOT relaunch GPU training until user resolves power/hardware** (mitigations: `sudo nvidia-smi -i 0 -pl 100`, smaller batch, UPS, off-box).
  - **Safe CPU progress (no GPU load):** evaluated recovered e19 on the same 4 NACT cases as v1 via fixed evaluator on CPU. Result: MSE 0.3987, LPIPS 0.1704, SSIM-tumor -0.1341, FRD 7.631, AUROC-c 0.75, AUROC-tumor 0.8125, Dice 0.5719, HD95 99.97. e16→e19 is **marginal/a wash**; bottleneck is the per-image min-max normalization bridge (squished z-range, negative SSIM-tumor), not schedule length. Artifact: `experiments/phase1b/finetune_pix2pixhd_v2/v2_e19_recovery_comparison_v1.{json,md}`. Decision: `no_promotion_exploratory_only`. n=64 LOSO still pending (needs GPU for speed; CPU n=64 nnU-Net is hours).
  - **GPU 1 last-resort FAILED (user-directed):** ran the e19 n=64 eval on GPU 1 (RTX 3080 Ti) as a probe; server hard-crashed AGAIN at **11:25:58** (~56s auto power-on, same logless signature, ~172W draw, 42/64 predictions done). **Both GPUs now crash the box → system-wide power fault, not GPU-specific.** Per user decision ("안되면 다른 머신"), GPU compute must move OFF-BOX. CLAUDE.md GPU rule + memory `server-power-crash-gpu-load` updated. No further on-box GPU jobs.
- [x] Evaluate recovered e19 on `splits/phase1b_loso_nact_v1.json` (n=64) with fixed evaluator — **completed on CPU (crash-safe)**. Result: MSE 0.0386, LPIPS 0.1413, SSIM-tumor -0.14, FRD 12.38, AUROC-c 0.705, **AUROC-tumor 0.4475 (sub-random)**, **Dice 0.368**, HD95 116.7. n=4 was misleadingly optimistic (Dice 0.57→0.37, AUROC-tumor 0.81→0.45 at n=64). Decision `no_promotion_exploratory_only`. Artifact: `experiments/phase1b/finetune_pix2pixhd_v2/e19_nact64_loso_v1.json`. (Snapshot selection sweep + DUKE-out fold need the other machine.)
- [ ] Write v2 comparison + promotion/no-promotion decision; then DUKE-out fold if NACT-out is encouraging.
- [ ] Investigate negative SSIM-tumor (z-score<->[-1,1] per-image min-max bridge) if it persists.

### Phase 2 — latent diffusion (CC-Net style: frozen SD VAE + concat-conditioning + fixed invertible norm)
- [x] Flip Phase 2 go/no-go gate to go (user-directed + concrete metric gap); `experiments/phase2/go_no_go_decision.json` decision=go; recorded in STRATEGY §4.2.
- [x] Scaffold: diffusers 0.37.1 installed (torch unchanged), staged `sd-vae-ft-mse` VAE (SD2.1 gated -> MIT-licensed substitute), wrote `src/phase2/latent_diffusion.py` (encode/train/eval) with fixed invertible z-score norm + per-channel latent standardization; smoke test `src/phase2/tests/test_latent_diffusion.py` -> 4 passed (CPU).
- [x] Encode full-dataset latents — **done on CPU (crash-safe)**: 1153 real cases → `experiments/phase2/ldm_v1/latents.pt` (1153×2 × 4×64×64 fp16, finite) + `norm_stats.json` (z-bounds [-0.5,12], per-channel latent mean/std). Validates the Phase 2 pipeline on real variable-shape data; deterministic + reusable (other machine loads directly, skips re-encode).
- [ ] Train conditional UNet + eval on NACT-64 LOSO — **needs GPU on another machine** (CPU diffusion training is hours of underfit/misleading output; not run).
- [ ] Compare Phase 1A vs Phase 2 per metric group; sweep until stable — **needs the other machine.**

### Hard blocker (terminal for this box): all safe on-box progress exhausted
Both GPUs hard-crash the server under load (4× on 2026-05-30). Maximal CRASH-SAFE progress is done: Phase 1A n=64 LOSO eval (real result, no_promotion); Phase 2 scaffold + smoke (4/4) + full-dataset encode. Remaining goal work (Phase 1A schedule completion, Phase 2 training, cross-phase sweep to stable performance) requires a GPU on another machine. Port assets (~3.2G): preprocessed mha 1.2G, 00023 696M, eval models 1.2G, **latents.pt 72M (Phase 2 ready-to-train)**, recovered FT checkpoints 4.1G (optional). Awaiting user machine access.
