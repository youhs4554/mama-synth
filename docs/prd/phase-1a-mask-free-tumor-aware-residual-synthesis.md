...
We need a first development phase that is both practically implementable on a single RTX A4500 20GB GPU and strong enough to support a task-specific contribution claim. The phase must establish a reproducible training/evaluation pipeline, preserve mask-free submission inference, and produce a submission candidate only when local hold-out evidence supports it.

## Solution

Build Phase 1A as **mask-free tumor-aware residual synthesis**: a 2D U-Net residual regressor that predicts the enhancement residual from the pre-contrast slice, uses ground-truth tumor masks only in training losses, and outputs a synthetic post image for evaluation and submission. The model must preserve native spatial size by padding internally and cropping back before output.
...
5. As a researcher, I want the Phase 1A claim framed as a task-specific adaptation rather than a novel architecture, so that the contribution is accurate and defensible.
...
7. As a researcher, I want the reference GAN treated as the primary performance baseline, so that improvements are measured against a meaningful comparison point.
...
21. As an evaluator, I want submission candidate promotion to require no major group regression, so that a model does not overfit one metric group.
22. As an evaluator, I want at least two metric groups to improve or remain useful, so that Phase 1A candidates are balanced.
23. As an evaluator, I want the 4-group rank mean to improve over the primary performance baseline, so that the candidate aligns with the challenge ranking philosophy.
...
26. As a future issue author, I want architecture details such as U-Net depth and optimizer choices left to implementation issues, so that the PRD stays focused on stable contracts.
27. As a future issue author, I want GAN strengthening, predicted-mask conditioning, and diffusion excluded from Phase 1A, so that the first implementation remains small and testable.

## Implementation Decisions

- Phase 1A will implement **mask-free tumor-aware residual synthesis**.
- The first backbone will be a 2D U-Net residual regressor.
...
- Native image size must be preserved. The model may pad internally to a U-Net-compatible multiple, but must crop back to the original size before producing synthetic post.
...
- A Phase 1A submission candidate must satisfy: no major metric-group regression, at least two metric groups improving or remaining useful, improved 4-group rank mean over the primary performance baseline, and passing submission smoke tests.
- The primary performance baseline is the reference GAN submission once its weights and inference path are reproducible.
- The identity output is a lower-bound benchmark and submission smoke-test artifact, not the primary performance baseline.
- Phase 1A should support a task-specific, evaluation-aligned contribution claim, not a claim of a novel architecture.
...
- Promotion-gate tests should verify candidate acceptance/rejection from synthetic metric summaries.
- Submission smoke tests should verify output creation, readability, 2D shape, float32 dtype, plausible z-score range, and metadata preservation.
